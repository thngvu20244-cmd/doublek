    ...
    if request.method == "POST":
        ...
        if not username or not raw_pass:
            ...
            try:
                ...
                message = "<p style='color:red;'>Username exists</p>"
...
def admin():
    ...
    return f"""
    <body style="background:#0f2027;color:white;padding:40px;font-family:Arial;">
        <h2>Admin Panel</h2>
        <table border="1" cellpadding="10" style="background:white;color:black;">
        <tr><th>ID</th><th>User</th><th>Role</th><th>Status</th><th>Action</th></tr>
        {rows}
        </table>
...
def change_password():
    ...
    if request.method == "POST":
        ...
        with sqlite3.connect(DATABASE) as conn:
            ...
            if user and check_password_hash(user[0], old):
                new_hash = generate_password_hash(new)
                c.execute("UPDATE users SET password=? WHERE id=?", (new_hash, session["user_id"]))
                conn.commit()
                message = "<p style='color:green;'>Password changed</p>"
            else:
...
def page_template(title, message, register_link):
    link = '<p><a href="/register">Register</a></p>' if register_link else '<p><a href="/">Login</a></p>'
    ...
    button{{width:100%;padding:10px;border:none;border-radius:8px;background:#2c5364;color:white;}}
    </style>
    </head>
    <body>
    <div class="box">
